...
- 步骤2：构建`LightGBM`分类器，将catecopy变量设置为categorical_feature，训练并记录下分别记录下验证集AUC精度。
- 步骤3：学习网格搜索原理，使用GridSearchCV完成其他超参数搜索，其他超参数设置可以选择`learning_rate`、`num_leaves`等。
- 步骤4：学习随机搜索原理，使用GridSearchCV完成其他超参数搜索，其他超参数设置可以选择`learning_rate`、`num_leaves`等。
- 步骤5：学习贝叶斯调参原
---
