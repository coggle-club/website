...
-  **步骤3（扩展）** ：在任务2中我们保存了`json`版本的树模型，其中一家包含了每棵树的结构，你可以手动读取后，试试吗？
...
-  **步骤3** ：构建`LightGBM`分类器，在`fit`函数中将catecopy变量设置为`categorical_feature`，训练并记录下分别记录下验证集AUC精度。
...
-  **步骤5** ：学习随机搜索原理，使用`GridSearchCV`完成其他超参数搜索，其他超参数设置可以选择`learning_rate`、`num_leaves`等。
