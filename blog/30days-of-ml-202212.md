...
- 任务4：协同过滤思路（召回阶段）
    - 思路1：[Iterm-CF思路](https://blog.csdn.net/lukeyyanghang/article/details/87358019)
    - 思路2：[User-CF思路](https://blog.csdn.net/jiangdongxiaobawang/article/details/104155673)
    - 思路3：内容相似度召回思路（提示，通过embedding计算文章的相似度）
    - 思路4：文章热度排序（统计用户当前点击时文章的热度）
    - 思路5：对比思路1-思路4下召回的文章的MRR得分。
    - 参考资料：[https://tianchi.aliyun.com/notebook-ai/detail?postId=144452](https://tianchi.aliyun.com/notebook-ai/detail?postId=144452)
