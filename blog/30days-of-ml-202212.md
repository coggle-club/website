...
- 任务4：树模型与特征工程（排序阶段）
    ...
    - 步骤2：采样得到负样本，使用训练集点击的样本作为正样本，随机采样部分文章（如每个用户采样10篇）作为负样本。
    - 步骤3：训练得到初步的排序模型，记录下模型的AUC。
    - 参考资料：[https://tianchi.aliyun.com/notebook-ai/detail?postId=144453](https://tianchi.aliyun.com/notebook-ai/detail?postId=144453)

- 任务5：协同过滤思路（召回阶段）
    - 步骤1：尝试[Iterm-CF思路](https://blog.csdn.net/lukeyyanghang/article/details/87358019)
    - 步骤2：尝试[User-CF思路](https://blog.csdn.net/jiangdongxiaobawang/article/details/104155673)
    - 步骤3：尝试内容相似度召回思路（提示，通过embedding计算文章的相似度）
    - 步骤4：尝试文章热度排序（统计用户当前点击时文章的热度）
    - 步骤5：对比步骤1-步骤4下召回的文章的MRR得分。
    - 参考资料：
